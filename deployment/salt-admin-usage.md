...
For the validated lab, select its Supervisor kubeconfig and namespace explicitly:

```bash
kubectl --kubeconfig "$SSC_KUBECONFIG" -n "$SSC_NAMESPACE" exec -it deploy/ssc-salt-master -- sh
kubectl --kubeconfig "$SSC_KUBECONFIG" -n "$SSC_NAMESPACE" exec deploy/ssc-salt-master -- /opt/saltstack/salt/salt 'your-test-minion' test.ping
```

The `aria-config` examples below apply to local development. Change namespace
and Deployment names when using an adapted Supervisor overlay. Store durable
master configuration in Git/ConfigMaps and credentials in Secrets, not edits
inside a running container. See [operations](supervisor/operations.md).
