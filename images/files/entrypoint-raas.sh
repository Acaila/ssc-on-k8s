...
# A high internal port supports non-root runtimes that cannot grant effective
# NET_BIND_SERVICE. Services/proxies can still publish HTTPS on port 443.
RAAS_LISTEN_PORT="${RAAS_LISTEN_PORT:-443}"
case "${RAAS_LISTEN_PORT}" in
  ''|*[!0-9]*) echo "RAAS_LISTEN_PORT must be an integer from 1 to 65535" >&2; exit 1 ;;
esac
if [ "${#RAAS_LISTEN_PORT}" -gt 5 ] || [ "${RAAS_LISTEN_PORT}" -lt 1 ] || [ "${RAAS_LISTEN_PORT}" -gt 65535 ]; then
  echo "RAAS_LISTEN_PORT must be an integer from 1 to 65535" >&2
  exit 1
fi
sed -i -E "s#^port:.*#port: ${RAAS_LISTEN_PORT}#" /etc/raas/raas
