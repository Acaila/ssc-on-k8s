...
This is phase 1 of the primary workflow: build locally from entitled inputs,
then [deploy directly to Supervisor](../deployment/supervisor/README.md).
Compose is an optional local smoke test, not a required intermediate platform.
...
## 11. Publish for Supervisor

For Supervisor, publishing to the private registry is required. Follow the
[OCI/Harbor guide](../deployment/supervisor/registry.md) for project permissions,
registry CA trust, image naming, PostgreSQL/Redis mirrors and pull credentials.
Apply your generated overlay directly; native pods pull the referenced images.

For optional local Minikube validation only, `minikube image load` can replace
registry publication. Supervisor cannot use a workstation's local image store.

## Native Podman build alternative

The wrapper scripts currently call Docker. You can also use
Podman directly with the same Dockerfiles after staging the installer:

```bash
podman build --platform linux/amd64 -f images/Dockerfile.raas -t localhost/ssc-raas:8.18.3 .
podman build --platform linux/amd64 -f images/Dockerfile.salt-master -t localhost/ssc-salt-master:3006-lts .
```

No Podman Kubernetes cluster is required. Use the same engine for building,
logging into the registry and pushing; Docker and Podman use separate local
image stores. The Docker preflight wrapper does not validate Podman.

## Reproducibility boundary

The current Salt Dockerfile selects the latest available 3006 release and some
base/dependency references are mutable. Rebuilding later can change the output.
Record resolved package versions and pushed image digests per release. Pinning
base images and dependencies is remaining work; a version-looking tag alone
does not make this build reproducible. Keep installer binaries and entitled
images within the permitted private distribution boundary.

## Notes

- `RAAS_LISTEN_PORT` defaults to 443; the Supervisor overlay selects 8443 for
  the non-root native-pod runtime while the public Service stays on 443.
